...
if 1
	ix = 10:nx-10;
	im clf, im([xtrue(ix,:); xup0(ix,:); xcp1(ix,:); xcg1(ix,:)], [0 8]), cbar
	title ''
	nrms(xup0, xtrue) % huge due to spatial distortion along y!
	nrms(xcp1, xtrue)
	nrms(xcg1, xtrue)
	...
	for ii=1:4
		text(xpos(ii), 0, label{ii}, 'horiz', 'center')
	end
	put = @(k,x) text(xpos(k), ny, ...
		sprintf('NRMSE = %.0f%%', 100*nrms(x,xtrue)), 'horiz', 'center');
	put(2, xup0)
	put(3, xcp1)
	put(4, xcg1)
	axis off

...
% generate data
if ~isvar('yi'), printm 'setup pwls_sps_example'
	f.count = 2e5;
	em_wls_test_setup
	W = diag_sp(wi(:));
prompt
...
if 1
	...
	im(4, xsps(:,:,end), 'PWLS-SPS', clim), cbar horiz
	im(5, xsos(:,:,end), 'PWLS-SPS-OS', clim), cbar horiz
	efbp = xfbpn - xtrue;
	epcg = xpcgn - xtrue;
	esps = xsps(:,:,end) - xtrue;
	esos = xsos(:,:,end) - xtrue;
	...
	im(9, esps, elim), cbar horiz, titlef('NRMSE %4.1f%%', nsps)

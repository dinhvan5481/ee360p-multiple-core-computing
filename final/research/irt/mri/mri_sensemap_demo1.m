...
%	ftrue = downsample2(ftrue, 2); % now 64^2
	[nx ny] = size(ftrue);
...
if ~isvar('shat1'), printm 'ratio'
	...
	alim = [];
	im(5, 'hsv', angle(shat1), '$\angle$ ratio', alim)
	im(8, 'hsv', angle(shat1-smap), '$\angle$ err', alim), cbar
	drawnow
end
...
if ~isvar('shat2'), printm 'qpls via cholesky'
	...
	im(6, 'hsv', angle(shat2), '$\angle$ qpls:chol', alim)
	im(9, 'hsv', angle(shat2-smap), '$\angle$ err', alim), cbar
	drawnow
end
...
if 1
	...
	im(2, 'hsv', angle(zlj), '$\angle$ zlj', alim)
	im(3, 'hsv', angle(smap), '$\angle$ smap', alim)
	im(4, 'hsv', angle(shat1), '$\angle$ ratio', alim)
	im(7, 'hsv', afun(shat1, smap), '$\angle$ err', alim)
	im(5, 'hsv', angle(shat2), '$\angle$ qpls:chol', alim)
	im(8, 'hsv', afun(shat2, smap), '$\angle$ err', alim)
	im(6, 'hsv', angle(shat3), '$\angle$ qpls:iter', alim)
	im(9, 'hsv', afun(shat3, smap), '$\angle$ err', alim)
	im(1, 'hsv', angle(ftrue), '$\angle$ ftrue', alim), colorbar
end
